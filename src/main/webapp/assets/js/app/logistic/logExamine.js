define(['ajaxPackage', 'timePicker', 'select', 'table', 'jqueryConfirm'],
    function (Lprapm, timePicker) {
        ...
        // 加载日期下来框
        timePicker.picker("#createELTime", "#endELTime");
        ...
        var operateEvent = { //要放在commonrow之前，因为是赋值函数，要置前
            ...
            'click .remove': function (event, value, row, index) {
                ...
                if (row.logState == "已回复" || row.logState == "已出发" || row.isSure == "是") {
                    $.confirm({
                        animation: 'rotateYR',
                        closeAnimation: 'rotate',
                        backgroundDismiss: true,
                        content: '物流询价已经回复或已经签订合同,不能撤销审核',
                    });
                } else {
                    $.confirm({
                        closeIcon: true,
        ...
        function editTable(row) {
            if (row.oeState == "审核中") {
                $("#addELForm").find('input[name="oePerson"]').val(username);
        ...
        $("#submitELBtn").click(function (event) {
            /* 点击提交按钮 */
            submitData();
        });
        ...
        function searchParams() {
            var sendParams = $("#searchELForm").serializeArray();
